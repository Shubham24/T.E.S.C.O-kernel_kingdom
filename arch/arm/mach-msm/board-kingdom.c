...
#define KINGDOM_GPIO_PS_HOLD	(29) /*ACPU GPIO Number 29*/
...
int __init kingdom_init_panel(void);

static unsigned int skuid;
unsigned int kingdom_get_skuid(void)
{
	return skuid;
}

static unsigned int engineerid;
unsigned int kingdom_get_engineerid(void)
{
	return engineerid;
}
...
static struct akm8975_platform_data compass_platform_data = {
	...
	.intr_pin = PM8058_GPIO_PM_TO_SYS(KINGDOM_GPIO_COMPASS_INT),
};
...
static struct i2c_board_info i2c_Sensors_devices[] = {
	{
		...
		.irq = MSM_GPIO_TO_INT(PM8058_GPIO_PM_TO_SYS(KINGDOM_GPIO_COMPASS_INT)),
	},
	{
		I2C_BOARD_INFO(BMA250_I2C_NAME, 0x32 >> 1),
		.platform_data = &gsensor_platform_data,
		.irq = MSM_GPIO_TO_INT(PM8058_GPIO_PM_TO_SYS(KINGDOM_GPIO_GSENSOR_INT)),
	},
	{
...
static int pm8058_gpios_init(void)
{
	...
	/* gpio01 */
	struct pm8xxx_gpio_init_info gpio01 = {
		PM8058_GPIO_PM_TO_SYS(KINGDOM_GPIO_TP_INT_N),
		{
        ...
        /* gpio02 */
	struct pm8xxx_gpio_init_info gpio02 = {
		PM8058_GPIO_PM_TO_SYS(KINGDOM_GPIO_TP_INT_N_CPU),
		{
	...
	/* gpio 21 */
	struct pm8xxx_gpio_init_info gpio21 = {
		PM8058_GPIO_PM_TO_SYS(KINGDOM_TP_RSTz),
		{
			.direction      = PM_GPIO_DIR_OUT,
			.output_buffer  = PM_GPIO_OUT_BUF_CMOS,
			.output_value   = 1,
	...
	struct pm8xxx_gpio_init_info gpio18 = {
		PM8058_GPIO_PM_TO_SYS(KINGDOM_AUD_EP_EN),
		{
			.direction      = PM_GPIO_DIR_OUT,
			.output_buffer  = PM_GPIO_OUT_BUF_CMOS,
			.output_value   = 0,
	...
	struct pm8xxx_gpio_init_info gpio19 = {
		PM8058_GPIO_PM_TO_SYS(KINGDOM_AUD_SPK_EN),
		{
			.direction      = PM_GPIO_DIR_OUT,
			.output_buffer  = PM_GPIO_OUT_BUF_CMOS,
			.output_value   = 0,
	...
	struct pm8xxx_gpio_init_info gpio24 = {
		PM8058_GPIO_PM_TO_SYS(KINGDOM_GREEN_LED),
		{
			.direction      = PM_GPIO_DIR_OUT,
			.output_buffer  = PM_GPIO_OUT_BUF_CMOS,
			.output_value   = 1,
	...
	struct pm8xxx_gpio_init_info gpio25 = {
		PM8058_GPIO_PM_TO_SYS(KINGDOM_AMBER_LED),
		{
			.direction      = PM_GPIO_DIR_OUT,
			.output_buffer  = PM_GPIO_OUT_BUF_CMOS,
			.output_value   = 1,
	...
	struct pm8xxx_gpio_init_info wimax_host_wakeup = {
		PM8058_GPIO_PM_TO_SYS(KINGDOM_WiMAX_HOST_WAKEUP),
		{
			.direction      = PM_GPIO_DIR_IN,
			.pull           = PM_GPIO_PULL_NO,
			.vin_sel        = PM8058_GPIO_VIN_S3,
	...
	/*HEADSET*/
	struct pm8xxx_gpio_init_info kim_headset_gpios[] = {
		{
			PM8058_GPIO_PM_TO_SYS(KINGDOM_AUD_REMO_PRES),
			{
				.direction      = PM_GPIO_DIR_IN,
				.output_buffer  = 0,
				.output_value   = 0,
		...
		{
			PM8058_GPIO_PM_TO_SYS(KINGDOM_AUD_REMO_EN),
			{
				.direction      = PM_GPIO_DIR_OUT,
				.output_buffer  = PM_GPIO_OUT_BUF_CMOS,
				.output_value   = 0,
	...
	/* gpio01 */
	rc = pm8xxx_gpio_config(gpio01.gpio, &gpio01.config);
	if (rc) {
		pr_err("%s PMIC_GPIO_01 config failed\n", __func__);
		return rc;
	}

        /* gpio 2 */
        rc = pm8xxx_gpio_config(gpio02.gpio, &gpio02.config);
        if (rc) {
                pr_err("%s PMIC_GPIO_02 config failed\n", __func__);
                return rc;
        }

	/* gpio 21 */
	rc = pm8xxx_gpio_config(gpio21.gpio, &gpio21.config);
	if (rc) {
		pr_err("%s PMIC_GPIO_21 config failed\n", __func__);
		return rc;
	}
	...
	if (rc) {
		pr_err("%s KINGDOM_AUD_EP_EN config failed\n", __func__);
		return rc;
	}
	rc = pm8xxx_gpio_config(gpio19.gpio, &gpio19.config);
	if (rc) {
		pr_err("%s KINGDOM_AUD_SPK_EN config failed\n", __func__);
		return rc;
	}
	...
	for (i = 0; i < ARRAY_SIZE(kim_headset_gpios); i++) {
		rc = pm8xxx_gpio_config(kim_headset_gpios[i].gpio, &kim_headset_gpios[i].config);
		if (rc) {
			pr_err("%s PMIC_GPIO_Headset (%d)config failed\n", __func__, i);
			return rc;
		}
	}

	keypad_gpio.gpio = KINGDOM_VOL_UP;
	pm8xxx_gpio_config(keypad_gpio.gpio, &keypad_gpio.config);
	keypad_gpio.gpio = KINGDOM_VOL_DN;
	pm8xxx_gpio_config(keypad_gpio.gpio, &keypad_gpio.config);
...
static struct tps65200_platform_data tps65200_data = {
	.gpio_chg_int = PM8058_GPIO_IRQ(PM8058_IRQ_BASE, KINGDOM_GPIO_CHG_INT),
};
...
#endif
#ifdef CONFIG_S5K3H2YX
static int Kingdom_s5k3h2yx_vreg_on(void)
{
  int rc;
...
static int Kingdom_s5k3h2yx_vreg_off(void)
{
#if 0
  return 0;
#else
...
#ifdef CONFIG_OV8830
static int Kingdom_ov8830_vreg_on(void)
{
	int rc = 0;
...
static int Kingdom_ov8830_vreg_off(void)
{
	int rc = 0;
...
static int Kingdom_s5k6aafx_vreg_on(void)
{
  ...
  mdelay(1);
  /* digital 1V8 */
  rc = camera_sensor_power_enable("gp13", 1800);
...
static int Kingdom_s5k6aafx_vreg_off(void)
{
 ...
 /* didital 1V8 */
  ...
  mdelay(1);
  /* digital 1V8 */
  rc = camera_sensor_power_disable("gp13");
...
static int config_kingdom_camera_on_gpios(void)
{
...
static void Kingdom_maincam_clk_switch(void){
	int rc = 0;
	pr_info("[CAM] Doing clk switch (s5k4e5yx)\n");
...
#ifdef CONFIG_S5K6AAFX
static void Kingdom_seccam_clk_switch(void){
...
static struct msm_camera_sensor_info msm_camera_sensor_ov8830_data = {
	...
	.vcm_pwd = CAM1_VCM_PWD,
	.vcm_enable = 1,
	.camera_power_on = Kingdom_ov8830_vreg_on,
...
static struct msm_camera_sensor_info msm_camera_sensor_s5k6aafx_data = {
	...
	.camera_clk_switch = Kingdom_seccam_clk_switch,
	.pdata = &camera_device_data,
...
static struct tpa2051d3_platform_data tpa2051d3_platform_data = {
	.gpio_tpa2051_spk_en = KINGDOM_AUD_SPK_EN,
};
...
static void __init kingdom_init_marimba(void)
{
#if 0
...
static void config_flashlight_gpios(void)
{
	uint32_t flashlight_gpio_table[] = {
		PCOM_GPIO_CFG(KINGDOM_GPIO_FLASHLIGHT_TORCH, 0, GPIO_OUTPUT,
						GPIO_NO_PULL, 0),
		PCOM_GPIO_CFG(KINGDOM_GPIO_FLASHLIGHT_FLASH, 0, GPIO_OUTPUT,
						GPIO_NO_PULL, 0),
	};
...
int64_t kingdom_get_usbid_adc(void)
{
...
void config_kingdom_usb_id_gpios(bool output)
{
	if (output) {
...
static struct cable_detect_platform_data cable_detect_pdata = {
	...
	.get_adc_cb		= kingdom_get_usbid_adc,
};
...
static void __init kingdom_init_irq(void)
{
	msm_init_irq();
}
#if 0
...
static unsigned int kingdom_sdc2_slot_type = MMC_TYPE_MMC;
static struct mmc_platform_data msm7x30_sdc2_data = {
	...
	.msmsdcc_fmid	= 24576000,
	.msmsdcc_fmax	= 49152000,
	.slot_type		= &kingdom_sdc2_slot_type,
	.nonremovable	= 1,
	.emmc_dma_ch	= 7,
};
#endif
...
#if 0
static unsigned int kingdom_sdc3_slot_type = MMC_TYPE_SDIO_WIFI;
static struct mmc_platform_data msm7x30_sdc3_data = {
	...
	.msmsdcc_fmid	= 24576000,
	.msmsdcc_fmax	= 49152000,
	.slot_type		= &kingdom_sdc3_slot_type,
	.nonremovable	= 0,
};
#endif
#endif
...
#ifdef CONFIG_MMC_MSM_SDC4_SUPPORT
static unsigned int kingdom_sdc4_slot_type = MMC_TYPE_SD;
static struct mmc_platform_data msm7x30_sdc4_data = {
	...
	.msmsdcc_fmid	= 24576000,
	.msmsdcc_fmax	= 49152000,
	.slot_type		= &kingdom_sdc4_slot_type,
	.nonremovable	= 0,
};
#endif
...
static void __init msm7x30_init_mmc(void)
{
	...
	vreg_sd = vreg_get(NULL, "gp10");
	if (IS_ERR(vreg_sd)) {
		printk(KERN_ERR "%s: vreg get failed (%ld)\n",
		       __func__, PTR_ERR(vreg_sd));
		return;
	}
...
#define PM_QOS 1
...
#ifdef CONFIG_MMC_MSM_SDC1_SUPPORT
	/* SDCC1 : WiMAX SQN1210 is connected */
	/*
	if (machine_is_msm7x30_fluid()) {
	...
	*/
	ret = kingdom_init_mmc(system_rev);
	if (ret != 0)
		pr_crit("%s: Unable to initialize MMC (SDCC1)\n", __func__);
#endif
#ifdef CONFIG_MMC_MSM_SDC2_SUPPORT
	if (machine_is_msm8x55_svlte_surf())
...
#endif
#ifdef CONFIG_MMC_MSM_SDC4_SUPPORT
	sdcc_vreg_data[3].vreg_data = vreg_sd;
	sdcc_vreg_data[3].level = 2850;

#if PM_QOS
    msm7x30_sdc4_data.swfi_latency =
		msm_pm_data[MSM_PM_SLEEP_MODE_WAIT_FOR_INTERRUPT].latency;
#endif
...
void kingdom_add_usb_devices(void)
{
...
static void kingdom_te_gpio_config(void)
{
...
#if 0
	if (gpio_get_value(KINGDOM_CAM_ID) == 0) {
	...
	if (skuid == 0x2a302) {
		for (rc = 0; rc < ARRAY_SIZE(i2c_devices); rc++) {
			if (!strcmp(i2c_devices[rc].type, ATMEL_QT602240_NAME)) {
				strcpy(i2c_devices[rc].type, SYNAPTICS_3K_INCELL_NAME);
...
static void __init kingdom_reserve(void)
{
	reserve_info = &msm7x30_reserve_info;
	msm_reserve();
}

static void __init kingdom_allocate_memory_regions(void)
{
	void *addr;
	unsigned long size;
...
{
	kingdom_allocate_memory_regions();
}
...
MACHINE_START(KINGDOM, "kingdom")
	...
	.reserve = kingdom_reserve,
	.init_irq = kingdom_init_irq,
	.init_machine = kingdom_init,
	.timer = &msm_timer,
	.init_early = kingdom_init_early,
MACHINE_END
